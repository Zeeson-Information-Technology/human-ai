...
export default function InterviewerStartPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      if (user.role === "admin" || user.role === "company") {
        router.replace("/zuri/start/admin"); // or "/admin"
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      {/* === Content Area === */}
      <div className="relative mx-auto flex min-h-screen w-full max-w-5xl flex-col px-4 py-12 sm:px-6 lg:px-8">
      ...
      dark:text-black dark:shadow-white/10 cursor-pointer"
              onClick={() => {
                if (
                  ...
                  router.push("/zuri/start/login?role=company");
                } else if (user.role === "admin") {
                  router.push("/zuri/start/admin");
                } else {
                  router.push("/zuri/start/admin");
                }
              }}
      ...
      dark:bg-white/10 dark:text-gray-100 dark:hover:bg-white/20 cursor-pointer"
              onClick={() => {
                if (!user || String(user.role) !== "talent") {
                  router.push("/zuri/start/login?role=talent");
                } else {
                  router.push("/zuri/start/jobs");
                }
              }}
            >
